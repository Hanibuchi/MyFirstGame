...
namespace MyGame
{
	...
	public class Item : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IResourceHandler
	{
		...
		/// </summary>
		/// <returns></returns>
		protected ResourceManager.UIID SlotID;
		...
		protected virtual void Init()
		{
			...
			itemCapacity = Data.ItemCapacity;
		...
		**/
		...
		public void ShowItemAndHideUI()
		{
			...
			if (ItemSlot != null)
				ResourceManager.Release(SlotID, ItemSlot.gameObject);
			ItemSlot = null;
		}
		...
		{
			ItemSlot = ResourceManager.Get(SlotID).GetComponent<ItemSlot>();
		...
		/// <summary>
		/// このアイテムが拾われるとき必要な処理をする。
		/// </summary>
		/// <param name="owner">拾ったmob</param>
		public void OnPickedUp(MobManager owner)
		{
			RemovePrevItem();
			EnableComponentsOnCollected(false);
			Owner = owner;
